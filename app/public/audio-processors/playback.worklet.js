...
// ─── PCM playback worklet (SoundTouch time-stretch) ─────────────────────────
// Streams incoming Float32 mono PCM chunks through SoundTouch for pitch-
// preserved time-stretching. tempo=1.0 plays at native speed; tempo=0.5 plays
// half-speed without dropping pitch (proper time-stretch, not tape slowdown).
// Pitch can also be shifted independently if needed, but we lock it to 1.0.
class PCMPlaybackProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.st = new SoundTouch();
    // Speech-tuned WSOLA windows at 24kHz. SoundTouch's auto-mode picks
    // 100+ ms sequences at low tempo which audibly echoes syllables ("voice
    // doubling"). Smaller windows trade transient smearing for no echo.
    //   sequence 40ms · seek 15ms · overlap 8ms — typical voice settings.
    this.st.stretch.setParameters(24000, 40, 15, 8);
    this.st.tempo = 1.0;
    this.st.pitch = 1.0;
    // SoundTouch FIFO is stereo-interleaved. We feed mono by duplicating
    // each sample into L and R, then read L back out.
    this.outQueue = []; // Float32Array chunks of mono output ready to play
    this.outCursor = 0; // index into outQueue[0]
    ...
    this.port.onmessage = (e) => {
      if (e.data === "interrupt") {
        ...
        const v = Math.max(0.25, Math.min(4, Number(e.data.value) || 1));
        this.st.tempo = v;
      } else if (e.data instanceof Float32Array) {
        // Interleave mono → stereo for SoundTouch
        const mono = e.data;
        ...
        this.st.inputBuffer.putSamples(stereo, 0, mono.length);
        this.st.process();
        // Drain output into mono chunks
        const frames = this.st.outputBuffer.frameCount;
        if (frames > 0) {
          const out = new Float32Array(frames * 2);
          this.st.outputBuffer.receiveSamples(out, frames);
          const monoOut = new Float32Array(frames);
          for (let i = 0; i < frames; i++) monoOut[i] = out[i * 2];
          this.outQueue.push(monoOut);
        }
      }
    };
  }
  ...
  process(_inputs, outputs) {
    ...
    let i = 0;
    while (i < out.length && this.outQueue.length > 0) {
      const chunk = this.outQueue[0];
      const remaining = chunk.length - this.outCursor;
      const toCopy = Math.min(remaining, out.length - i);
      for (let j = 0; j < toCopy; j++) out[i + j] = chunk[this.outCursor + j];
      i += toCopy;
      this.outCursor += toCopy;
      if (this.outCursor >= chunk.length) {
        this.outQueue.shift();
        this.outCursor = 0;
      }
    }
